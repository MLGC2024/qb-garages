...
QBCore.Functions.CreateCallback('qb-garages:server:canDeposit', function(source, cb, plate, type, garage, state)
    ...
    --if type == 'house' and not exports['qb-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.Garages[garage].houseName) then --removed by pamela for ps-housing
    if type == 'house' and not exports['ps-housing']:IsOwner(source, garage) then -- added by pamela for ps-housing 
